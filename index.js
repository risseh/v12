...
api = ["https://apikey-rizqi.herokuapp.com/"]
simbol = ["•"]
...
case 'korea':
case 'japan':
ccn = await getBuffer(`https://apikey-bear.herokuapp.com/docs/cecan/${command}?apikey=KingOfBear`)
Lexxy.sendMessage(from, gfx, image, { quoted: mek, caption: `Random Cecan ${command}`})
break
case 'malaysia':
yh = await fetchJson(`https://apikey-bear.herokuapp.com/docs/cecan/malaysia?apikey=KingOfBear`)
...
Lexxy.sendMessage(from, gfx, image, { quoted: mek, caption: '*Logo By KingOfBear Officia*'})
break
case 'gfx3':
case 'gfx4':
if (args.length ==0) return reply(`Text Nya Mana? Contohnya\n${prefix+command} KingOfBear OFC`)
sticWait(from)
ini_txt1 = args[0]
ini_txt1 = args[1]
...
"listMessage": {
"title": '「 STORE MENU 」',
"description": `*Created By KingOfBear*`,
"buttonText": "𝙋𝙄𝙇𝙄𝙃 𝘿𝙄𝙎𝙄𝙉𝙄",
"listType": "SINGLE_SELECT",
"sections": [
{
...
case 'yuri':
case 'neko':
get = await fetchJson(`https://apikey-bear.herokuapp.com/docs/nsfw/${command}?apikey=KingOfBear`)
...
tk = args.join(" ")
ini = await fetchJson(`https://apikey-bear.herokuapp.com/docs/download/tiktok?url=${tk}&apikey=KingOfBear`)
sticLoad(from)
...
Lexxy.sendMessage(from, bf, image, { quoted: flexx, caption: 'Logo By KingOfBear Official' })
break
case 'kaneki':
case 'rem':
case 'lolimaker':
...
Lexxy.sendMessage('6288279268363@s.whatsapp.net', options, text, {quoted: flexx})
Lexxy.sendMessage('6288279268363@s.whatsapp.net', options, text, {quoted: flexx})
